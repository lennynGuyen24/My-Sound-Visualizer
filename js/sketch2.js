...
function setup() {
    ...
    colorMode(HSB, 255);
    fft=new p5.FFT();
    mic = new p5.AudioIn(); // Create a new mic instance
...
*/
    // toggle play/pause button https://emojidb.org/play-button-emojis
    playButton = createButton('▶ Play');
    playButton.parent(buttonSpace);
    playButton.addClass('playButton');
    playButton.mousePressed(() => {
   ...
   changeSongButton.mousePressed(()=> {
    ...
    currentSongIndex=(currentSongIndex+1) % songs.length;
    song =loadSound(songs[currentSongIndex], ()=> {
        song.setVolume(volumeSlider.value()); // Set the volume to the current slider value
        song.play();
        loop();
        songSelector.selected(currentSongIndex);
...
// Handle song selection
 ...
 // Tạo thanh điều chỉnh âm lượng và lưu vào window.volumeSlider
...
function draw() {
  ...
  /*  for (let boxes=0; boxes<num; boxes++) {
   ...
   if (amp > 200) {
    ...
    // Vẽ vòng tròn volume ở góc trái trên màn hình (overlay)
    let micVol = mic.getLevel();
    // nếu volume vượt ngưỡng thì tăng và giữ mức đó
    ...
    noFill();
    let hue = map(micVol, 0, 1, 180, 0); // màu từ xanh nhạt -> đỏ theo volume
    stroke(hue, 255, 255);
    ellipse(0, 0, radius);
    pop();
}
